...
class main {
    ...
    init() {
        ...
        this.setAttribute(gl, prog, 'position', plane.pos, 3);
        this.setAttribute(gl, prog, 'color', plane.col, 4);
        this.setAttribute(gl, prog, 'textureCoord', plane.tex, 2);
        this.index = plane.idx;
        const ibo = this.createIBO(gl, plane.idx);
        ...
        this.uLocation = {
            ...
            texture2: gl.getUniformLocation(
                prog,
                'texture2'
            ) as WebGLUniformLocation,
        };
        ...
        this.createTexture(gl, 'negx');
        this.createTexture(gl, 'negy');
    ...
    render(gl: WebGLRenderingContext, counter: number) {
        ...
        if (this.texGroup && this.texGroup['negx']) {
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, this.texGroup['negx']);
            gl.uniform1i(this.uLocation.texture1, 0);
        }
        if (this.texGroup && this.texGroup['negy']) {
            gl.activeTexture(gl.TEXTURE1);
            gl.bindTexture(gl.TEXTURE_2D, this.texGroup['negy']);
            gl.uniform1i(this.uLocation.texture2, 1);
        }
