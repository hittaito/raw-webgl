...
export class Torus {
    ...
    constructor(row: number, column: number, irad: number, orad: number) {
        this.init(row, column, irad, orad);
    }
    init(row: number, column: number, irad: number, orad: number): void {
        const pos: number[] = [];
        const col: number[] = [];
        const norm: number[] = [];
        const idx: number[] = [];
        [...new Array(row + 1)].forEach((_, i) => {
            ...
            [...new Array(column + 1)].forEach((_, j) => {
                ...
                const tx = (rcos * irad + orad) * Math.cos(tr);
                const ty = rsin * irad;
                const tz = (rcos * irad + orad) * Math.sin(tr);

                const rx = rcos * Math.cos(tr);
                const rz = rcos * Math.sin(tr);

                const tc = hsva((360 / column) * j, 1, 1, 1) as number[];
